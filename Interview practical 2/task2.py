...
class ListADT:
    ...
    def __init__(self, size=100):
        ...
        :pre-condiiton: size must be a positive integer
        :post-condition: An empty list of specified size is created. The size should be 40 or more.
    ...
    def insert(self, index, item):
        ...
        :return: None
        :pre-condition: index is within the range -len(self)-1 to len(self) and index is of type integer
        :post-condition: every item from index to the end of the list is shifted one space
                         ...
                         has increased by 1. If the list is full, increase the size to be 1.9 times the current size
        ...
        if self.is_full():
            size = math.ceil(1.9 * self.length)
            new = ListADT(size)
            for i in range(self.length):
                new.the_array[i] = self.the_array[i]
            self.the_array = new.the_array
    ...
    def delete(self, index):
        ...
        :post-condition: Every item to the right of given index is shifted one space to the left.
                         The length of the list is decreased by 1. If len(self) is less than a quarter of capacity,
                         shrink it to half its size (minimum size is 40)
        ...
        if not self.is_empty():
            ...
            self.length -= 1
            if self.length < (0.25 * len(self.the_array)):
                new_size = len(self.the_array) // 2
                if new_size < 40:
                    new_size = 40
                new = ListADT(new_size)
                for i in range(self.length):
                    new.the_array[i] = self.the_array[i]
                self.the_array = new.the_array
        else:
    ...
    def append(self, item):
        ...
        :complexity: best case is O(1) when no resizing needs to be done and worst case is O(n) where n is the
                     length of list
        """
        if self.is_full():
            size = math.ceil(1.9 * self.length)
