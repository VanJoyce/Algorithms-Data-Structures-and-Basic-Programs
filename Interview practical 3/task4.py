...
class HashTable:
    ...
    def __getitem__(self, key):
        ...
        if self.table[hash_value] is not None and self.table[hash_value][0] != key:  # collision, check probe chain
            n = 1
            i = (hash_value + n**2) % len(self.table)
            while i != hash_value:
...
def table_load_dictionary_statistics(max_time):
    """
    Creates a csv file 'output_task4.csv' and writes into it the statistics of each combination of hash base and
    ...
    with open('output_task4.csv', 'w', newline='') as file:
